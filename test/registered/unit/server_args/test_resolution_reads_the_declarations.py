...
# The one reader that has no resolved surface to read. The gateway binding is
# installed against a *released* sglang wheel, which has `resolve_once` but not
# `resolved_dict`; on such a wheel the field still carries the resolved value,
# so the fallback branch reads it on purpose. The preferred branch right above
# it uses `resolved_dict()` whenever the wheel offers one. A new entry here
# needs that kind of reason next to it.
