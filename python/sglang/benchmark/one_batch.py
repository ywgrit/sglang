...
def main(server_args, bench_args):
    ...
    # per-phase knob is raw input, so the value joins the record before
    # resolution reads it, through the copy helper: this record is unresolved,
    # so the copy is a plain one, and a field assignment is the shape the
    # mutation ratchet pins at zero.
    server_args = server_args.replace_resolved(
        "benchmark.one_batch", cuda_graph_max_bs_decode=max(bench_args.batch_size)
    )
